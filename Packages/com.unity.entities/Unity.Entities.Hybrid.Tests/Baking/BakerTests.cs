    ...
    public partial class BakerTests : BakingSystemFixtureBase
    {
        ...
        public void Baker_AddComponentObjectGeneric()
        {
            ...
            using (new BakerDataUtility.OverrideBakers(true, typeof(AddManagedComponentBakerTest)))
            {
                ...
                // GetEntitiesForBakers returns an UnsafeHashSet — ToNativeArray order is not guaranteed;
                // verify that one entity has Field==2 and the other Field==4.
                #pragma warning disable 0618 // managed API obsolete; internal/test caller still needs it.
                ...
                Assert.IsTrue((comp1.Field == 2 && comp2.Field == 4) || (comp1.Field == 4 && comp2.Field == 2));
            }
        }
#endif
        ...
        public void AddComponentGeneric()
        {
            ...
            using (new BakerDataUtility.OverrideBakers(true, typeof(AddComponentGeneric_SecondaryValidEntity)))
            {
                ...
                // GetEntitiesForBakers returns an UnsafeHashSet — ToNativeArray order depends on entity
                // index values and is not guaranteed; find the additional entity by its component.
                var additionalEntity = m_Manager.HasComponent<ComponentTest1>(entities[0]) ? entities[0] : entities[1];
                Assert.IsTrue(m_Manager.HasComponent<ComponentTest1>(additionalEntity));

                ComponentTest1 comp1 = m_Manager.GetComponentData<ComponentTest1>(additionalEntity);
        ...
        public void AddSharedComponentGeneric()
        {
                ...
                typeof(AddSharedComponentGeneric_SecondaryValidEntity)))
            {
                ...
                // GetEntitiesForBakers returns an UnsafeHashSet — ToNativeArray order is not guaranteed;
                // verify that one entity has Field==1 and the other Field==3.
                #pragma warning disable 0618 // managed API obsolete; internal/test caller still needs it.
                UnmanagedSharedComponent comp0 = m_Manager.GetSharedComponentManaged<UnmanagedSharedComponent>(entities[0]);
                UnmanagedSharedComponent comp1 = m_Manager.GetSharedComponentManaged<UnmanagedSharedComponent>(entities[1]);
                #pragma warning restore 0618
                Assert.IsTrue((comp0.Field == 1 && comp1.Field == 3) || (comp0.Field == 3 && comp1.Field == 1));
            }
        }
