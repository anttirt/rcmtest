...
namespace Unity.Entities
{
        ...
        ITypedVisit<Entity>
    {
        ...
        void IPropertyVisitor.Visit<TContainer, TValue>(Property<TContainer, TValue> property, ref TContainer srcContainer)
        {
            ...
            // For types whose reflected property bag is empty (e.g. Entity whose backing field is
            // internal), the container-visitor path would produce a zeroed default. Use the typed
            // visitor instead so those types are copied by value directly.
            if (this is ITypedVisit<TValue> typed)
            {
                typed.Visit(property, ref srcContainer, ref dstValue);
            }
            else
            {
        ...
        // Entity is a packed struct (ulong m_RawData) with no public fields; the reflected
        // property bag is empty, so the normal container-visitor path zeroes it. Copy directly.
        ...
        void CloneValue<TValue>(ref TValue dstValue, TValue srcValue)
        {
            ...
            // For types handled by ITypedVisit (e.g. Entity, which is a container with an empty
            // property bag), copy the value directly. Without this, the container path below would
            // visit an empty property bag and produce a zeroed-out default.
            if (this is ITypedVisit<TValue>)
            {
                dstValue = srcValue;
                return;
            }
