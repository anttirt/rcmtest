...
namespace Unity.Entities.Editor
{
    ...
    public static class ArchetypeSearchProvider
    {
        ...
        static ArchetypesMemoryDataRecorder s_Recorder;
        static NativeList<ulong> s_ArchetypesStableHash;
        static NativeList<ArchetypeMemoryData> s_ArchetypesMemoryData;
        static MemoryProfilerTreeViewItemData[] s_ArchetypesDataSource;
        ...
        static void SetupQueryEngine()
        {
            ...
            SearchBridge.AddFilter<string, MemoryProfilerTreeViewItemData>(s_QueryEngine, "t", OnTypeFilter, new[] { ":", "=" });
        }
        ...
        static void OnEnable()
        {
            if (s_Recorder != null)
                return;
            s_Recorder = new ArchetypesMemoryDataRecorder();
            s_ArchetypesStableHash = new NativeList<ulong>(64, Allocator.Persistent);
            s_ArchetypesMemoryData = new NativeList<ArchetypeMemoryData>(64, Allocator.Persistent);
        }

        static void OnDisable()
        {
            if (s_ArchetypesMemoryData.IsCreated)
                s_ArchetypesMemoryData.Dispose();
            if (s_ArchetypesStableHash.IsCreated)
                s_ArchetypesStableHash.Dispose();
            s_Recorder?.Dispose();
            s_Recorder = null;
        }
        ...
        {
            var worldsData = s_Recorder.WorldsData.Distinct().ToDictionary(x => x.SequenceNumber, x => x);
            var archetypesData = s_Recorder.ArchetypesData.Distinct().ToDictionary(x => x.StableHash, x => x);

            var componentsSet = new HashSet<ArchetypeComponentData>();
            foreach (var component in s_Recorder.ArchetypeComponentsData)
            ...
            foreach (var archetypeMemoryData in s_Recorder.ArchetypesMemoryData)
            {
        ...
        {
            s_Recorder.Record();
            if (!ArchetypesMemoryDataRecorder.MemCmp(s_ArchetypesStableHash.AsArray(), s_Recorder.ArchetypesStableHash) ||
                !ArchetypesMemoryDataRecorder.MemCmp(s_ArchetypesMemoryData.AsArray(), s_Recorder.ArchetypesMemoryData))
            {
                s_ArchetypesDataSource = GetTreeViewData().ToArray();
                s_ArchetypesStableHash.CopyFrom(s_Recorder.ArchetypesStableHash);
                s_ArchetypesMemoryData.CopyFrom(s_Recorder.ArchetypesMemoryData);
            }

            if (s_ArchetypesDataSource == null)
                s_ArchetypesDataSource = GetTreeViewData().ToArray();
        }
        ...
        static IEnumerable<SearchItem> FetchItems(SearchContext context, SearchProvider provider)
        {
            ...
            TickArchetypeSource();
            var results = s_ArchetypesDataSource?.Where(a => a.EntityCount > 0) ?? Enumerable.Empty<MemoryProfilerTreeViewItemData>();
            if (query != null)
            {
        ...
        static IEnumerable<SearchProposition> FetchPropositions(SearchContext context, SearchPropositionOptions options)
        {
            ...
            foreach (var l in SearchBridge.GetPropositionsFromListBlockType(typeof(QueryArchetypeComponentTypeBlock)))
                yield return l;
        }
    ...
    [QueryListBlock("Component", "t", "t", "=")]
    class QueryArchetypeComponentTypeBlock : QueryListBlock
    {
        public QueryArchetypeComponentTypeBlock(IQuerySource source, string id, string value, QueryListBlockAttribute attr)
        ...
        public override IEnumerable<SearchProposition> GetPropositions(SearchPropositionFlags flags)
        {
            ...
            foreach (var cn in SearchUtils.componentNames)
            {
                ...
                yield return new SearchProposition(category, $"{id}{op}{cn}", replacement: name, help: string.Format(L10n.Tr("Component type: {0}"), cn), type: GetType(), data: name);
            }
        }
    }
}
